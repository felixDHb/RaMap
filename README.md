...
&nbsp;
  ...
  - hese low-SNR spectra are considered too noisy and could degrade the quality of the analysis. They are exported as _Excluded_spaectra.txt_. Output:
...
Spectra saved as 'Excluded_spectra.txt'." with the following format:
...
- The corresponding spectra are saved as 'Inconsistency_spectra.txt' with the following format:
  - **First column:** wavenumber
...
The file already exists in the specified results folder.  
Please confirm if you want to overwrite the existing file.
...
## Tips
- typical values for PL threshold: 0.8 < $R_{pl}$ < 1
- typical values for SNR threshold: 2 < $SNR_{th}$ < 5
