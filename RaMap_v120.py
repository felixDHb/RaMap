...
map_file = askopenfilename(
    title="Choose a Raman mapping file",
    filetypes=[("Supported files", "*.txt *.csv")]
)
map_folder_path = os.path.dirname(map_file)
...
if os.path.isfile(os.path.join(map_folder_path, f"dict_ref_{map_file_name}.json")):
    ...
    with open(os.path.join(map_folder_path, f"dict_ref_{map_file_name}.json"), "r") as json_file:
        dict_ref = json.load(json_file)

    for key in dict_ref:
        dict_ref[key]["ref_shifts"] = np.array(dict_ref[key]["ref_shifts"])
        dict_ref[key]["ref_spectrum"] = np.array(dict_ref[key]["ref_spectrum"])
else:
    root = Tk()
    root.withdraw()

    file_paths = askopenfilenames(
        title="Choose reference Raman spectra",
        filetypes=[
    ...
    for i, path in enumerate(file_paths):
        ...
        compound_key = input("Compound name: ")
        chemical_formula = input("Chemical formula: ")
        color_desc = input(f"Plot color (press Enter for system color {system_color}): ").strip().lower()
        ...
        if color_desc == "":
            ...
            # simple color names ("red" etc.)
            else:
                color = color_desc

        raman_shifts, intensities = load_ref(path)
        ...
        dict_ref_json[compound_key] = {
            "ref_shifts": raman_shifts.tolist(),
            "ref_spectrum": intensities.tolist(),
            "chemical formula": f"${chemical_formula}$",
            "plot color": color
        }

        dict_ref_path = os.path.join(map_folder_path, f"dict_ref_{map_file_name}.json")

        with open(dict_ref_path, "w") as json_file:
            json.dump(dict_ref_json, json_file)

        print(f"Dictionary saved in {dict_ref_path}.")
...
path_results = os.path.join(map_folder_path, f'RaMap_results-{map_file_name}')
...
def Save_Figure(filename, decision):
    ...
    # The value is set to 'y' to enable the saving of figures for the Raman mapping results.
    '''
